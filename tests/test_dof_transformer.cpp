...
int main() {
    ...
    // 2. Kiểm tra tính trực giao phải: T * T^T = I_n
    NAV_TEST_ASSERT(t_3dof.is_orthogonal(), "T_3dof * T_3dof^T must equal Identity_3!");

    // 3. Thu giảm ma trận khối lượng Fossen: M_r = T * M * T^T
    ...
    // Kiểm tra đường chéo: m + X_udot = 17.0, m + Z_wdot = 26.1, Izz + N_rdot = 0.47
    ...
    // Các phần tử ngoài đường chéo bằng 0 khi r_G nằm trên trục z
    ...
    // 4. Thu giảm ma trận Coriolis Fossen: C_r = T * C * T^T
    ...
    // Ma trận Coriolis thu giảm phải duy trì tính phản đối xứng: C_r + C_r^T = 0
    ...
    // Với ROV 3-DOF có sway=roll=pitch=0, C_3 đồng nhất triệt tiêu về 0
    ...
    // 5. Thu giảm ma trận cản Fossen: D_r = T * D * T^T
    ...
    // Kiểm tra tính tiêu tán năng lượng của ma trận cản thu giảm: v_r^T * D_r * v_r > 0
    ...
    // 7. Khởi tạo từ tên các bậc tự do (dof_names)

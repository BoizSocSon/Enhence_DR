...
/**
 * @brief Mô-đun chuyên trách xây dựng ma trận biến đổi bậc tự do T thuộc R^{n x 6},
 * và thực hiện các phép nhân ma trận với các thành phần trong phương trình động lực Fossen:
 *   M_r   = T * M * T^T
 *   C_r   = T * C * T^T
 *   D_r   = T * D * T^T
 *   g_r   = T * g
...
public:
    ...
    /// Lấy chỉ số hàng (0..n-1) của một DOF hoạt động trong không gian thu giảm (-1 nếu không hoạt động)
    ...
    /// Thu giảm véc-tơ lực điều khiển tổng quát 6x1: tau_r = T * tau thuộc R^{n x 1}
    ...
    /// Mở rộng véc-tơ thu giảm n x 1 về véc-tơ 6D: v_6d = T^T * v_r + (I - T^T*T) * v_constrained

...
Vector6d MassMatrixEvaluator::solve(const Vector6d& b) const {
    // Solve M * x = b via Cholesky decomposition (LLT) or fallback to LDLT
    Eigen::LLT<Matrix6d> llt(M_total_);
    if (llt.info() == Eigen::Success) {
        return llt.solve(b);
    }
    // Fallback to LDLT for semi-definite/near-singular or slightly non-SPD
